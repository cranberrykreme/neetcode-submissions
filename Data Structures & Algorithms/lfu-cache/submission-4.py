...
class LFUCache:
    ...
    def __init__(self, capacity: int):
        ...
        self.dict_counter_lists = {} # lists head and tail for each counter index
        self.set_counter_indicies = set() # lists all of the counter indicies
    ...
    def get(self, key: int) -> int:
        ...
        while node.next and node.next.counter <= node.counter:
            self._bubble_node_along_list(node)
        return node.val
    ...
    def _bubble_node_along_list(self, node: Node):
        old_prev = node.prev
        old_next = node.next
